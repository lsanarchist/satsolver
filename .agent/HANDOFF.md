...
- `cp-001`, `cp-002`, `cp-003`, `sat-001`, `tool-001`, `perf-001`, `perf-002`, `perf-003`, `perf-004`, `perf-005`, `perf-006`, `perf-007`, `perf-008`, `perf-009`, `perf-010`, `perf-011`, `perf-012`, `perf-013`, `perf-014`, `perf-015`, `perf-016`, `perf-017`, `perf-018`, `perf-019`, `perf-020`, `perf-021`, `perf-022`, `perf-023`, `perf-024`, `perf-025`, `perf-026`, `perf-027`, `perf-028`, `perf-029`, `perf-030`, `perf-031`, `perf-032`, `perf-033`, `perf-034`, `perf-035`, `perf-036`, `perf-037`, `perf-038`, `perf-039`, `perf-040`, `perf-041`, `perf-042`, `perf-043`, `perf-044`, `perf-045`, `perf-046`, and `perf-047` are complete.
- There is no active in-progress task; the next deterministic task is `perf-048`.

## What Changed This Run

- Closed `perf-047` as a retained measurement-only run with no solver change.
- Added profiler-only exact `index 7` versus `index 8+` counters inside the exact `sub10 step-3` deep-overwrite `index 7+` lane, plus targeted regression coverage.
- The dense anchors split `17,756` exact `index 7+` hits into `5,616` at exact index `7` versus `12,140` at `index 8+`, so the remaining deep tail is still dominated by `index 8+`.
- The real supplemental target trio (`uuf125-010`, `uf125-01`, `uf125-010`) split `160` exact `index 7+` hits into `45` at exact index `7` versus `115` at `index 8+`, while `jnh10` and `jnh1` stayed at zero exact `index 7+` hits.

## Current Focus

- Start `perf-048` next: test one bounded solver-core candidate only on the exact `sub10 step-3` deep-overwrite `index 8+` tail while keeping exact `index 7`, exact `index 6`, exact `index 5`, exact `index 4`, exact `index 3`, exact `index 2`, shallow, and last-slot behavior on the retained baseline path.

## Recommended Next Tasks

- `perf-048` — test the exact index-8-plus deep overwrite tail after the perf-047 profile

## Verification From This Run

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q` — passed (`23/23` green, including the new exact `index 7` versus `index 8+` deep-tail split test)
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf` — passed and reported dense-anchor exact `index 7+` deep overwrites `5,616` at exact index `7` versus `12,140` at `index 8+`, plus supplemental target-trio `45` versus `115` while `jnh10` and `jnh1` stayed at zero exact `index 7+` hits
- `python tools/agent_queue_check.py` — passed after the final control-plane sync; queue now resolves to `current_or_next_task='perf-048'`
...
- Do not update `benchmark_summary.md` or `experiments.jsonl` for `perf-047`; this run kept no solver change.
...
- `perf-045` now shows that the remaining exact `index 6+` tail is itself dominated by exact source index `7+`, `perf-046` shows that even the exact `index 7+` aggregate is still too broad for the retained pop-first rewrite, and `perf-047` shows that the surviving `index 7+` mass is itself dominated by exact `index 8+`, so the next bounded solver-core edit should stay on that exact `index 8+` tail and should not widen back out before testing it directly.
...
- `perf-048` should test only the exact `sub10 step-3` deep-overwrite `index 8+` tail before any wider solver-core rewrite is attempted.
