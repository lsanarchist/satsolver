...
- `cp-001`, `cp-002`, `cp-003`, `sat-001`, `tool-001`, `perf-001` through `perf-061` are complete.
- There is no active in-progress task; the next deterministic task is `perf-062`.

## What Changed This Run

- Closed `perf-061` as a retained measurement-only run with no solver change.
- Added profiler-only exact `index 14` versus `index 15+` deep-overwrite counters plus regression coverage in `tools/profile_solver.py` and `tests/test_profile_solver.py`.
- The dense anchors split exact `index 14+` deep overwrites `358` at exact index `14` versus `1,334` at `index 15+`.
- The supplemental target trio only showed non-zero exact `index 14+` traffic on `uuf125-010`, which split `0` at exact index `14` versus `13` at `index 15+`; `uf125-01`, `uf125-010`, `jnh10`, and `jnh1` stayed at zero exact `index 14+` hits.

## Current Focus

- Start `perf-062` next: test one bounded solver-core candidate only on the exact `sub10 step-3` deep-overwrite `index 15+` lane, while keeping exact `index 14` and all shallower retained baseline behavior unchanged.

## Recommended Next Tasks

- `perf-062` — test the exact index-15-plus deep overwrite tail after the perf-061 profile

## Verification From This Run

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q` — passed (`30/30` green, including the new exact `index 14` versus `index 15+` deep-tail split test)
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf` — passed and showed exact `index 15+` dominating exact `index 14` on the dense anchors (`1,334` vs `358`) and on the only non-zero supplemental target-trio case, `uuf125-010` (`13` vs `0`)
- `python tools/agent_queue_check.py` — passed after the final control-plane sync; queue now resolves to `current_or_next_task='perf-062'`
...
- `perf-062` should test one bounded solver-core candidate only on the exact `sub10 step-3` deep-overwrite `index 15+` lane before any broader learnt-large bookkeeping rewrite is retried.
