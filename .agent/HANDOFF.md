...
- `cp-001`, `cp-002`, `cp-003`, `sat-001`, `tool-001`, `perf-001` through `perf-055` are complete.
- There is no active in-progress task; the next deterministic task is `perf-056`.

## What Changed This Run

- Closed `perf-055` as a measurement-only profiling run with no solver change.
- Added profiler-only exact `index 11` versus `index 12+` counters plus regression coverage inside the exact `sub10 step-3` deep-overwrite `index 11+` lane.
- The dense anchors `large/test_6.cnf` and `special/hard.cnf` combined to `1,133` exact `index 11` hits versus `3,057` `index 12+` hits.
- The real supplemental target trio `uuf125-010`, `uf125-01`, and `uf125-010` combined to `9` exact `index 11` hits versus `25` `index 12+` hits, while `jnh10` and `jnh1` stayed at zero exact `index 11+` hits.

## Current Focus

- Start `perf-056` next: test one bounded solver-core candidate only on the exact `sub10 step-3` deep-overwrite `index 12+` lane, while keeping exact `index 11` and shallower retained baseline behavior unchanged.

## Recommended Next Tasks

- `perf-056` — test the exact index-12-plus deep overwrite tail after the perf-055 profile

## Verification From This Run

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q` — passed (`27/27` green, including the new exact `index 11` versus `index 12+` deep-tail split test)
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf` — passed and reported `large/test_6.cnf` exact `index 11+` deep overwrites `422` at exact index `11` versus `1,141` at `index 12+`
...
- `python tools/profile_solver.py satlib_more/uuf125-010.cnf` — passed and reported exact `index 11+` deep overwrites `9` at exact index `11` versus `25` at `index 12+`
- `python tools/profile_solver.py satlib_more/uf125-01.cnf` — passed and reported zero exact `index 11+` deep overwrites
- `python tools/profile_solver.py satlib_more/uf125-010.cnf` — passed and reported zero exact `index 11+` deep overwrites, confirming supplemental target-trio totals `9` versus `25`
- `python tools/agent_queue_check.py` — passed after the final control-plane sync; queue now resolves to `current_or_next_task='perf-056'`
- `python tools/codex_verify.py` — passed after the final control-plane sync (`87/87` tests green plus compile, queue, checker, and wrapper smoke checks)
...
- `perf-056` should test one bounded solver-core candidate only on the exact `sub10 step-3` deep-overwrite `index 12+` lane before any broader learnt-large relocation rewrite is attempted again.
