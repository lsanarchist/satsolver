...
- `cp-001`, `cp-002`, `cp-003`, `sat-001`, `tool-001`, `perf-001`, `perf-002`, `perf-003`, `perf-004`, `perf-005`, `perf-006`, `perf-007`, `perf-008`, `perf-009`, `perf-010`, `perf-011`, `perf-012`, `perf-013`, `perf-014`, `perf-015`, `perf-016`, `perf-017`, `perf-018`, `perf-019`, `perf-020`, `perf-021`, `perf-022`, `perf-023`, `perf-024`, `perf-025`, `perf-026`, `perf-027`, `perf-028`, `perf-029`, `perf-030`, `perf-031`, `perf-032`, `perf-033`, `perf-034`, `perf-035`, `perf-036`, `perf-037`, `perf-038`, and `perf-039` are complete.
- There is no active in-progress task; the next deterministic task is `perf-040`.

## What Changed This Run

- Closed `perf-039` as a measurement-only profiling run with no solver change.
- Added profiler-only exact `index 3` versus `index 4+` deep-overwrite counters inside `tools/profile_solver.py`, with regression coverage in `tests/test_profile_solver.py`.
- The exact `index 3+` deep-overwrite lane is now clearly dominated by `index 4+`, not exact index `3`. Dense anchors combined `23,468` exact index-`3` deep overwrites versus `55,828` at `index 4+`, and the supplemental target trio `uuf125-010`, `uf125-01`, and `uf125-010` combined `200` versus `449`.
- `satlib_more/jnh10.cnf` and `satlib_more/jnh1.cnf` stayed guardrails again, with zero exact `index 3+` deep-overwrite traffic in this run.

## Current Focus

- Start `perf-040` next: test one bounded solver-core candidate only on the exact `sub10 step-3` deep-overwrite `index 4+` tail, while keeping exact index `3`, exact index `2`, shallow, and last-slot behavior on the retained path.

## Recommended Next Tasks

- `perf-040` — target the exact deep overwrite index-4-plus tail after the source-index split

## Verification From This Run

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q` — passed (`19/19` green, including the new exact `index 3` versus `index 4+` deep-tail split coverage)
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf` — passed and reported that exact `index 3+` deep overwrites skew heavily to `index 4+` on both dense anchors and the supplemental target trio
- `python tools/agent_queue_check.py` — passed after the final control-plane sync; queue now resolves to `current_or_next_task='perf-040'`
...
- `perf-040` should touch only exact `sub10 step-3` deep overwrites at source index `4+`, leaving exact index `3`, exact index `2`, shallow, and last-slot behavior on the retained baseline path unless fresh evidence clearly justifies widening the lane.
