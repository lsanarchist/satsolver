...
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-063`, by measuring how the surviving exact `sub10 step-3` deep-overwrite `index 15+` lane splits between exact source index `15` and source index `16+`
- Assumptions:
  - `perf-062` rejected the whole exact `index 15+` aggregate, so this run should stay measurement-only and leave the retained solver path unchanged.
  - The next bounded candidate should only target the dominant surviving tail once the profiler proves whether exact `index 15+` is mostly exact `index 15` or still mostly `index 16+`.
  - A retained-noop outcome is valid if the new counters only narrow the lane and produce no solver change.
- Escalations: none

### Plan

- [x] Mark `perf-063` in progress in `PLANS.md` and the control plane.
- [x] Add profiler-only exact `index 15` versus `index 16+` deep-overwrite counters plus regression coverage.
...
- passed (`31/31` green, including the new exact `index 15` versus `index 16+` deep-tail split test)
...
- passed and reported dense-anchor exact `index 15+` deep overwrites `300` at exact index `15` versus `1,034` at `index 16+`; the only non-zero supplemental target-trio traffic was `uuf125-010` at `3` versus `10`, while `uf125-01`, `uf125-010`, `jnh10`, and `jnh1` stayed at zero exact `index 15+` hits
- `python tools/profile_solver.py satlib_more/uuf125-010.cnf | rg -o "learnt_large_success_sub10_step3_source_pop_overwrite_deep_index(15_plus|15|16_plus)=[0-9]+"`
- passed and confirmed the only non-zero supplemental target-trio traffic was `13` overall, split `3` at exact index `15` versus `10` at `index 16+`
- `python tools/profile_solver.py satlib_more/uf125-01.cnf | rg -o "learnt_large_success_sub10_step3_source_pop_overwrite_deep_index(15_plus|15|16_plus)=[0-9]+"`
- passed and confirmed zero exact `index 15+` deep overwrites on `uf125-01`
- `python tools/profile_solver.py satlib_more/uf125-010.cnf | rg -o "learnt_large_success_sub10_step3_source_pop_overwrite_deep_index(15_plus|15|16_plus)=[0-9]+"`
- passed and confirmed zero exact `index 15+` deep overwrites on `uf125-010`
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-064'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync (`91/91` tests green plus compile, queue, checker, and wrapper smoke checks)
- `git diff --check`
- passed after the final control-plane sync
...
- Added profiler-only exact `index 15` versus `index 16+` counters and regression coverage, then measured that the surviving exact `index 15+` tail is still dominated by `index 16+`, so no solver change was kept.

### Remaining risks

- The surviving tail is now smaller and sparser again, so the next exact `index 16+` candidate still needs dense-anchor, focused seven-case, and supplemental-slice gates before any keep.

## 2026-03-23 `perf-062-index15plus-deep-overwrite-bookkeeping`

- Status: completed
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-062`, by testing one bounded solver-core candidate that only touches exact `sub10 step-3` learnt-large non-last deep-overwrite removals when the source watcher index is `15+`
- Assumptions:
  - `perf-061` showed that the surviving exact `index 14+` deep-overwrite tail is still dominated by exact `index 15+` on both the dense anchors and the only non-zero supplemental target-trio case, `uuf125-010`.
  - The smallest faithful candidate is still the earlier pop-first watcher-removal rewrite, but gated only to exact `sub10 step-3` learnt-large non-last deep-overwrite removals at source index `15+` so the edit stays same-search and bookkeeping-only.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-062` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one exact `sub10 step-3` deep-overwrite `index 15+` bookkeeping candidate against the dense anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate (`90/90` tests green plus compile, queue, checker, and wrapper smoke checks)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf062_index15plus_baseline.78SPGP/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- candidate regressed the dense anchor pair two-order average (`20.8689s -> 21.0896s`)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf062_index15plus_baseline.78SPGP/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- candidate regressed the focused seven-case gate (`25.4931s -> 25.9160s`)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf062_index15plus_baseline.78SPGP/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- candidate regressed the supplemental slice (`0.3546s -> 0.3779s`), so there was no reason to run the repeat-aware exact-CLI full suite

### Outcome

- Tested the bounded exact `index 15+` pop-first watcher-removal rewrite, reverted it, and kept no solver change because all three early gates regressed overall.

### Remaining risks

- Even the exact `index 15+` aggregate is still too broad for this bookkeeping rewrite, so the next run should stay measurement-only and split it into exact `index 15` versus `index 16+` before another solver-core edit.
...
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-057`, by profiling how the surviving exact `sub10 step-3` deep-overwrite `index 12+` lane splits between exact source index `12` and source index `13+` across the dense anchors and the supplemental `satlib_more` guard slice
- Assumptions:
  - `perf-056` already rejected the pop-first rewrite across the whole exact `index 12+` deep-overwrite aggregate, so this run should stay measurement-only and identify whether the remaining tail is really concentrated at exact index `12` or in the deeper `index 13+` tail.
  - The current profiler already separates exact `step-3` last-slot, overwrite, shallow, deep, exact `index 2`, aggregate `index 3+`, exact `index 3`, aggregate `index 4+`, exact `index 4`, aggregate `index 5+`, exact `index 5`, aggregate `index 6+`, exact `index 6`, aggregate `index 7+`, exact `index 7`, aggregate `index 8+`, exact `index 8`, aggregate `index 9+`, exact `index 9`, aggregate `index 10+`, exact `index 10`, aggregate `index 11+`, exact `index 11`, and aggregate `index 12+` cases, so the missing information should be a profiler-only split inside the `index 12+` bucket plus regression coverage.
...
- [x] Mark `perf-057` in progress in the control plane and record the active profiling task in `PLANS.md`.
- [x] Add profiler-only exact `index 12` versus `index 13+` deep-overwrite counters plus regression coverage, then profile the dense anchors and supplemental guard slice.
- [x] Close the measurement run, queue the next deterministic task, verify the final state, and commit.

### Verification

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q`
- passed (`28/28` green, including the new exact `index 12` versus `index 13+` deep-tail split test)
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- passed and reported `large/test_6.cnf` exact `index 12+` deep overwrites `305` at exact index `12` versus `836` at `index 13+`
- `python tools/profile_solver.py special/hard.cnf | rg -o "learnt_large_success_sub10_step3_source_pop_overwrite_deep_index(11_plus|11|12_plus|12|13_plus)=[0-9]+"`
- passed and reported exact `index 12+` deep overwrites `486` at exact index `12` versus `1,430` at `index 13+`, confirming dense-anchor totals `791` versus `2,266`
- `python tools/profile_solver.py satlib_more/uuf125-010.cnf | rg -o "learnt_large_success_sub10_step3_source_pop_overwrite_deep_index(11_plus|11|12_plus|12|13_plus)=[0-9]+"`
- passed and reported exact `index 12+` deep overwrites `8` at exact index `12` versus `17` at `index 13+`
- `python tools/profile_solver.py satlib_more/uf125-01.cnf | rg -o "learnt_large_success_sub10_step3_source_pop_overwrite_deep_index(11_plus|11|12_plus|12|13_plus)=[0-9]+"`
- passed and reported zero exact `index 12+` deep overwrites
- `python tools/profile_solver.py satlib_more/uf125-010.cnf | rg -o "learnt_large_success_sub10_step3_source_pop_overwrite_deep_index(11_plus|11|12_plus|12|13_plus)=[0-9]+"`
- passed and reported zero exact `index 12+` deep overwrites, confirming supplemental target-trio totals `8` versus `17`
- `python tools/profile_solver.py satlib_more/jnh10.cnf | rg -o "learnt_large_success_sub10_step3_source_pop_overwrite_deep_index(11_plus|11|12_plus|12|13_plus)=[0-9]+"`
- passed and reported zero exact `index 12+` deep overwrites
- `python tools/profile_solver.py satlib_more/jnh1.cnf | rg -o "learnt_large_success_sub10_step3_source_pop_overwrite_deep_index(11_plus|11|12_plus|12|13_plus)=[0-9]+"`
- passed and reported zero exact `index 12+` deep overwrites
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-058'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync (`88/88` tests green plus compile, queue, checker, and wrapper smoke checks)
- `git diff --check`
- passed after the final control-plane sync

### Outcome

- Added profiler-only exact `index 12` versus `index 13+` counters and regression coverage for the remaining exact `sub10 step-3` deep-overwrite tail.
- Measured that the surviving exact `index 12+` tail is still dominated by `index 13+`, not exact `index 12`, on both dense anchors: `large/test_6.cnf` plus `special/hard.cnf` combined to `791` exact `index 12` hits versus `2,266` `index 13+` hits.
- The real supplemental target trio also favors the deeper tail: `uuf125-010`, `uf125-01`, and `uf125-010` combined to `8` exact `index 12` hits versus `17` `index 13+` hits, while `jnh10` and `jnh1` stayed at zero exact `index 12+` hits.
- The queue therefore advances to `perf-058`, which should test one bounded solver-core candidate only on the exact `index 13+` tail.

### Remaining risks

- This run was measurement-only, so there is still no retained speedup.
- The exact `index 13+` aggregate is narrower than `index 12+`, but it may still hide focused-slice losses until a bounded solver-core candidate is tested against the dense anchors, the focused seven-case slice, and the supplemental `satlib_more` guard slice.

## 2026-03-23 `perf-056-index12plus-deep-overwrite-bookkeeping`

- Status: completed
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-056`, by testing one bounded solver-core candidate that only touches exact `sub10 step-3` learnt-large non-last deep-overwrite removals when the source watcher index is `12+`
- Assumptions:
  - `perf-055` showed that the surviving exact `index 11+` deep-overwrite tail is still dominated by exact `index 12+` on both dense anchors and the real supplemental target trio, so this run should keep exact `index 11`, exact `index 10`, exact `index 9`, exact `index 8`, exact `index 7`, exact `index 6`, exact `index 5`, exact `index 4`, exact `index 3`, exact `index 2`, shallow, and last-slot behavior on the retained baseline path.
  - The smallest faithful candidate is the earlier pop-first watcher-removal rewrite, but gated only to exact `sub10 step-3` learnt-large non-last deep-overwrite removals at source index `12+` so the edit stays same-search and bookkeeping-only.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-056` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one exact `sub10 step-3` deep-overwrite `index 12+` bookkeeping candidate against the dense anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate (`87/87` tests green plus compile, queue, checker, and wrapper smoke checks)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf056_index12plus_baseline.MuCvzT/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- candidate regressed the dense anchor pair two-order average (`20.7783s -> 21.4315s`)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf056_index12plus_baseline.MuCvzT/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- candidate rejected on the focused seven-case gate (`25.3949s -> 26.2127s`), with the largest stable givebacks on `large/test_6.cnf`, `medium/test_4.cnf`, and `large/test_8.cnf`
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf056_index12plus_baseline.MuCvzT/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- supplemental slice improved (`0.3757s -> 0.3602s`), but not enough to overcome the dense-anchor and focused-slice regressions
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-057'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync
- `git diff --check`
- passed after the final control-plane sync
...
- Tested one bounded exact `sub10 step-3` deep-overwrite `index 12+` bookkeeping candidate by applying the pop-first watcher-removal rewrite only on that lane, then reverted it.
- The exact `index 12+` aggregate is still too broad for a retained keep: dense anchors regressed from `20.7783s` to `21.4315s`, and the focused seven-case gate regressed from `25.3949s` to `26.2127s`.
- The positive supplemental `satlib_more` signal shows some real movement remains in this lane, but it is still mixed enough that the next run should split exact `index 12+` into exact source index `12` versus `index 13+` before another solver-core edit.

### Remaining risks

- No solver speedup was retained in this run.
- The exact `index 12+` aggregate still contains useful supplemental-slice signal, so a broader “lane exhausted” conclusion would be premature until the tail is split once more.

## 2026-03-23 `perf-055-index11plus-source-index-profile`

- Status: completed
- Task family: native-only learnt-large profiling refresh
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-055`, by profiling how the surviving exact `sub10 step-3` deep-overwrite `index 11+` lane splits between exact source index `11` and source index `12+` across the dense anchors and the supplemental `satlib_more` guard slice
- Assumptions:
  - `perf-054` already rejected the pop-first rewrite across the whole exact `index 11+` deep-overwrite aggregate, so this run should stay measurement-only and identify whether the remaining tail is really concentrated at exact index `11` or in the deeper `index 12+` tail.
  - The current profiler already separates exact `step-3` last-slot, overwrite, shallow, deep, exact `index 2`, aggregate `index 3+`, exact `index 3`, aggregate `index 4+`, exact `index 4`, aggregate `index 5+`, exact `index 5`, aggregate `index 6+`, exact `index 6`, aggregate `index 7+`, exact `index 7`, aggregate `index 8+`, exact `index 8`, aggregate `index 9+`, exact `index 9`, aggregate `index 10+`, exact `index 10`, and aggregate `index 11+` cases, so the missing information should be a profiler-only split inside the `index 11+` bucket plus regression coverage.
  - A completed measurement-only outcome is valid if it names the dominant exact tail sublane and leaves the queue with one narrower next solver-core experiment.
- Escalations: none

### Plan

- [x] Mark `perf-055` in progress in the control plane and record the active profiling task in `PLANS.md`.
- [x] Add profiler-only exact `index 11` versus `index 12+` deep-overwrite counters plus regression coverage, then profile the dense anchors and supplemental guard slice.
- [x] Close the measurement run, queue the next deterministic task, verify the final state, and commit.

### Verification

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q`
- passed (`27/27` green, including the new exact `index 11` versus `index 12+` deep-tail split test)
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- passed and reported `large/test_6.cnf` exact `index 11+` deep overwrites `422` at exact index `11` versus `1,141` at `index 12+`
- `python tools/profile_solver.py special/hard.cnf`
- passed and reported exact `index 11+` deep overwrites `711` at exact index `11` versus `1,916` at `index 12+`, confirming dense-anchor totals `1,133` versus `3,057`
- `python tools/profile_solver.py satlib_more/uuf125-010.cnf`
- passed and reported exact `index 11+` deep overwrites `9` at exact index `11` versus `25` at `index 12+`
- `python tools/profile_solver.py satlib_more/uf125-01.cnf`
- passed and reported zero exact `index 11+` deep overwrites
- `python tools/profile_solver.py satlib_more/uf125-010.cnf`
- passed and reported zero exact `index 11+` deep overwrites, confirming supplemental target-trio totals `9` versus `25`
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-056'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync (`87/87` tests green plus compile, queue, checker, and wrapper smoke checks)
- `git diff --check`
- passed after the final control-plane sync

### Outcome

- Added profiler-only exact `index 11` versus `index 12+` counters and regression coverage for the remaining exact `sub10 step-3` deep-overwrite tail.
- Measured that the surviving exact `index 11+` tail is still dominated by `index 12+`, not exact `index 11`, on both dense anchors: `large/test_6.cnf` plus `special/hard.cnf` combined to `1,133` exact `index 11` hits versus `3,057` `index 12+` hits.
- The real supplemental target trio also favors the deeper tail: `uuf125-010`, `uf125-01`, and `uf125-010` combined to `9` exact `index 11` hits versus `25` `index 12+` hits, while `jnh10` and `jnh1` stayed at zero exact `index 11+` hits.
- The queue therefore advances to `perf-056`, which should test one bounded solver-core candidate only on the exact `index 12+` tail.

### Remaining risks

- This run was measurement-only, so there is still no retained speedup.
- The exact `index 12+` aggregate is narrower than `index 11+`, but it may still hide focused-slice losses until a bounded solver-core candidate is tested against the dense anchors, the focused seven-case slice, and the supplemental `satlib_more` guard slice.

## 2026-03-23 `perf-054-index11plus-deep-overwrite-bookkeeping`

- Status: completed
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-054`, by testing one bounded solver-core candidate that only touches exact `sub10 step-3` learnt-large non-last deep-overwrite removals when the source watcher index is `11+`
- Assumptions:
  - `perf-052` already rejected the pop-first rewrite across the whole exact `index 10+` aggregate, and `perf-053` now shows that the surviving exact `index 10+` tail is still dominated by exact `index 11+`, so this run should keep exact `index 10`, exact `index 9`, exact `index 8`, exact `index 7`, exact `index 6`, exact `index 5`, exact `index 4`, exact `index 3`, exact `index 2`, shallow, and last-slot behavior on the retained baseline path.
  - The smallest faithful candidate is the earlier pop-first watcher-removal rewrite, but gated only to exact `sub10 step-3` learnt-large non-last deep-overwrite removals at source index `11+` so the edit stays same-search and bookkeeping-only.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-054` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one exact `sub10 step-3` deep-overwrite `index 11+` bookkeeping candidate against the dense anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate (`86/86` tests green plus compile, queue, checker, and wrapper smoke checks)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf054_index11plus_baseline.hjAo1A/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- candidate improved the dense anchor pair two-order average (`21.7420s -> 20.8537s`)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf054_index11plus_baseline.hjAo1A/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- candidate rejected on the focused seven-case gate (`26.6291s -> 26.8779s`), with the largest stable givebacks on `special/hard.cnf`, `medium/test_4.cnf`, `medium/test_3.cnf`, and `satlib_more/uuf150-01.cnf`
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf054_index11plus_baseline.hjAo1A/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- supplemental slice improved (`0.3410s -> 0.3319s`), but not enough to overcome the primary-gate regression
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-055'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync
- `git diff --check`
- passed after the final control-plane sync
...
- Closed `perf-054` as a retained no-op with no solver change.
- Tested one bounded solver-core candidate in `satsolver_core.py`, mirrored in `tools/profile_solver.py`, that applied the earlier pop-first watcher-removal rewrite only to exact `sub10 step-3` learnt-large non-last deep-overwrite removals at source index `11+`, then reverted it after the focused seven-case gate regressed.
- The exact `index 11+` lane is still too broad: the dense anchor pair improved clearly and the supplemental `satlib_more` slice improved too, but the primary focused seven-case gate still regressed enough to stop before the repeat-aware exact-CLI full suite.
- The queue therefore advances to `perf-055`, which should stay measurement-only and split the exact `index 11+` deep-overwrite tail into exact source index `11` versus `index 12+` before another solver-core edit.

### Remaining risks

- The exact `index 11+` tail is the narrowest solver-core lane that still shows positive dense-anchor and supplemental signals, but `perf-054` proves that even this aggregate can still hide focused-slice losses. The next follow-up should not widen back out before that tail is split once more.

## 2026-03-23 `perf-053-index10plus-source-index-profile`

- Status: completed
- Task family: native-only learnt-large profiling refresh
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-053`, by profiling how the surviving exact `sub10 step-3` deep-overwrite `index 10+` lane splits between exact source index `10` and source index `11+` across the dense anchors and the supplemental `satlib_more` guard slice
- Assumptions:
  - `perf-052` already rejected the pop-first rewrite across the whole exact `index 10+` deep-overwrite aggregate, so this run should stay measurement-only and identify whether the remaining tail is really concentrated at exact index `10` or in the deeper `index 11+` tail.
  - The current profiler already separates exact `step-3` last-slot, overwrite, shallow, deep, exact `index 2`, aggregate `index 3+`, exact `index 3`, aggregate `index 4+`, exact `index 4`, aggregate `index 5+`, exact `index 5`, aggregate `index 6+`, exact `index 6`, aggregate `index 7+`, exact `index 7`, aggregate `index 8+`, exact `index 8`, aggregate `index 9+`, exact `index 9`, and aggregate `index 10+` cases, so the missing information should be a profiler-only split inside the `index 10+` bucket plus regression coverage.
  - A completed measurement-only outcome is valid if it names the dominant exact tail sublane and leaves the queue with one narrower next solver-core experiment.
- Escalations: none

### Plan

- [x] Mark `perf-053` in progress in the control plane and record the active profiling task in `PLANS.md`.
- [x] Add profiler-only exact `index 10` versus `index 11+` deep-overwrite counters plus regression coverage, then profile the dense anchors and supplemental guard slice.
- [x] Close the measurement run, queue the next deterministic task, verify the final state, and commit.

### Verification

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q`
- passed (`26/26` green, including the new exact `index 10` versus `index 11+` deep-tail split test)
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- passed and reported dense-anchor exact `index 10+` deep overwrites `1,649` at exact index `10` versus `4,190` at `index 11+`; the same run also confirmed that `jnh10` and `jnh1` stayed at zero exact `index 10+` hits
- `python tools/profile_solver.py satlib_more/uuf125-010.cnf`
- passed and reported exact `index 10+` deep overwrites `12` at exact index `10` versus `34` at `index 11+`
- `python tools/profile_solver.py satlib_more/uf125-01.cnf`
- passed and reported zero exact `index 10+` deep overwrites
- `python tools/profile_solver.py satlib_more/uf125-010.cnf`
- passed and reported exact `index 10+` deep overwrites `2` at exact index `10` versus `0` at `index 11+`, confirming supplemental target-trio totals `14` versus `34`
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-054'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync (`86/86` tests green plus compile, queue, checker, and wrapper smoke checks)
- `git diff --check`
- passed after the final control-plane sync
...
- The dense anchors split `1,649` exact `index 10` hits versus `4,190` exact `index 11+` hits (`28.24% / 71.76%`), and the real supplemental target trio split `14` versus `34` (`29.17% / 70.83%`), which is strong enough to justify one more bounded narrowing step.
- The queue therefore advances to `perf-054`, which should test one bounded solver-core candidate only on the exact `index 11+` deep-overwrite tail while keeping exact `index 10` and shallower retained baseline behavior unchanged.
...
## 2026-03-23 `perf-052-index10plus-deep-overwrite-bookkeeping`

- Status: completed
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-052`, by testing one bounded solver-core candidate that only touches exact `sub10 step-3` learnt-large non-last deep-overwrite removals when the source watcher index is `10+`
- Assumptions:
  - `perf-050` already rejected the pop-first rewrite across the whole exact `index 9+` aggregate, and `perf-051` now shows that the surviving exact `index 9+` tail is still dominated by exact `index 10+`, so this run should keep exact `index 9`, exact `index 8`, exact `index 7`, exact `index 6`, exact `index 5`, exact `index 4`, exact `index 3`, exact `index 2`, shallow, and last-slot behavior on the retained baseline path.
  - The candidate should stay same-search and bookkeeping-only: use the same replacement literal and append target, but swap only the watcher-list removal sequence on the exact `index 10+` lane.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-052` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one exact `sub10 step-3` deep-overwrite `index 10+` bookkeeping candidate against the dense anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate (`85/85` tests green plus compile, queue, checker, and wrapper smoke checks)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf052_index10plus_baseline.cJsalu/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- candidate improved the dense anchor pair two-order average (`21.9532s -> 21.7999s`)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf052_index10plus_baseline.cJsalu/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- candidate rejected on the focused seven-case gate (`26.5961s -> 27.1248s`), with the biggest givebacks on `special/hard.cnf`, `large/test_10.cnf`, and `large/test_8.cnf`
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf052_index10plus_baseline.cJsalu/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- supplemental slice was effectively flat-to-slightly positive (`0.3529s -> 0.3519s`), but not strong enough to offset the primary-gate regression
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-053'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync
- `git diff --check`
- passed after the final control-plane sync
...
- Closed `perf-052` as a retained no-op with no solver change.
- Tested one bounded solver-core candidate in `satsolver_core.py`, mirrored in `tools/profile_solver.py`, that applied the pop-first watcher-removal rewrite only to exact `sub10 step-3` learnt-large non-last deep-overwrite removals at source index `10+`, then reverted it after the same-day evidence stayed mixed.
- The exact `index 10+` lane is still too broad: the dense anchor pair improved slightly and the supplemental `satlib_more` slice was essentially flat, but the focused seven-case gate regressed clearly enough to stop before the repeat-aware exact-CLI full suite.
- The queue therefore advances to `perf-053`, which should stay measurement-only and split the exact `index 10+` tail into exact source index `10` versus `index 11+` before another solver-core edit.

### Remaining risks

- The surviving exact `index 10+` aggregate is still the narrowest measured solver-core lane, but `perf-052` shows that even this aggregate can hide focused-slice losses. The next follow-up should not widen back out before splitting that tail again.
...
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-050`, by testing one bounded solver-core candidate that only touches exact `sub10 step-3` learnt-large non-last deep-overwrite removals when the source watcher index is `9+`
- Assumptions:
  - `perf-048` already rejected the pop-first rewrite across the whole exact `index 8+` aggregate, and `perf-049` now shows that the surviving exact `index 8+` tail is still dominated by exact `index 9+`, so this run should keep exact `index 8`, exact `index 7`, exact `index 6`, exact `index 5`, exact `index 4`, exact `index 3`, exact `index 2`, shallow, and last-slot behavior on the retained baseline path.
  - The candidate should stay same-search and bookkeeping-only: use the same replacement literal and append target, but swap only the watcher-list removal sequence on the exact `index 9+` lane.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-050` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one exact `sub10 step-3` deep-overwrite `index 9+` bookkeeping candidate against the dense anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf050_index9plus_baseline.TgjnOA/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- candidate rejected on the dense anchor pair two-order average (`20.6525s -> 21.0735s`)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf050_index9plus_baseline.TgjnOA/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- essentially flat on the focused seven-case gate (`25.7157s -> 25.7140s`), but not enough to offset the denser negative signal elsewhere
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf050_index9plus_baseline.TgjnOA/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- candidate rejected on the supplemental slice (`0.3152s -> 0.3415s`), with the largest losses on `jnh10.cnf` and forward-order `uf125-010.cnf`
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-051'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync
- `git diff --check`
- passed after the final control-plane sync
...
- Tested one bounded solver-core candidate in `satsolver_core.py`, mirrored in `tools/profile_solver.py`, that applied the earlier pop-first watcher-removal rewrite only to exact `sub10 step-3` learnt-large non-last deep-overwrite removals at source index `9+`, then reverted it after the early gates failed to produce a clean keep signal.
- The exact `index 9+` aggregate is still too broad: the dense anchors regressed, the focused seven-case slice was only effectively neutral, and the supplemental `satlib_more` slice regressed clearly.
- The queue therefore advances to `perf-051`, which should stay measurement-only and split the exact `index 9+` tail into exact source index `9` versus `index 10+` before another solver-core edit.

### Remaining risks

- The surviving exact `index 9+` aggregate is still the narrowest measured solver-core lane, but `perf-050` shows that even this aggregate can hide losses across the dense anchors and the supplemental target trio. The next follow-up should not widen back out before splitting that tail again.

## 2026-03-23 `perf-049-index8plus-source-index-profile`

- Status: completed
- Task family: native-only learnt-large profiling refresh
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-049`, by profiling how the surviving exact `sub10 step-3` deep-overwrite `index 8+` lane splits between exact source index `8` and source index `9+` across the dense anchors and the supplemental `satlib_more` guard slice
- Assumptions:
  - `perf-048` already rejected the pop-first rewrite across the whole exact `index 8+` deep-overwrite aggregate, so this run should stay measurement-only and identify whether the remaining tail is really concentrated at exact index `8` or in the deeper `index 9+` tail.
  - The current profiler already separates exact `step-3` last-slot, overwrite, shallow, deep, exact `index 2`, aggregate `index 3+`, exact `index 3`, aggregate `index 4+`, exact `index 4`, aggregate `index 5+`, exact `index 5`, aggregate `index 6+`, exact `index 6`, aggregate `index 7+`, exact `index 7`, and aggregate `index 8+` cases, so the missing information should be a profiler-only split inside the `index 8+` bucket plus regression coverage.
  - A completed measurement-only outcome is valid if it names the dominant exact tail sublane and leaves the queue with one narrower next solver-core experiment.
- Escalations: none

### Plan

- [x] Mark `perf-049` in progress in the control plane and record the active profiling task in `PLANS.md`.
- [x] Add profiler-only exact `index 8` versus `index 9+` deep-overwrite counters plus regression coverage, then profile the dense anchors and supplemental guard slice.
- [x] Close the measurement run, queue the next deterministic task, verify the final state, and commit.

### Verification

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q`
- passed (`24/24` green, including the new exact `index 8` versus `index 9+` deep-tail split test)
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- passed and reported dense-anchor exact `index 8+` deep overwrites `3,793` at exact index `8` versus `8,347` at `index 9+`, plus supplemental target-trio exact `index 8+` deep overwrites `37` versus `78`; `jnh10` and `jnh1` stayed at zero exact `index 8+` hits
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-050'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync
- `git diff --check`
- passed after the final control-plane sync
...
- Added profiler-only exact `index 8` versus `index 9+` counters in `tools/profile_solver.py` plus regression coverage in `tests/test_profile_solver.py`, then kept the solver path unchanged.
- The surviving exact `sub10 step-3` deep-overwrite `index 8+` tail is still dominated by the deeper `index 9+` side on both the dense anchors and the real supplemental target trio, so the queue now narrows to an exact `index 9+` solver-core candidate instead of treating all `index 8+` cases as one bucket.
- The dense anchors split `31.24%` exact `index 8` versus `68.76%` exact `index 9+`, and the real supplemental target trio split `32.17%` versus `67.83%`, which is strong enough to justify the next narrowing step without another broad aggregate experiment.

### Remaining risks

- This run only sharpened the lane selection. The next solver-core experiment still has to clear the dense anchors, focused seven-case slice, and supplemental guard slice before it earns a full repeat-aware exact-CLI keep gate.

## 2026-03-23 `perf-048-index8plus-deep-overwrite-bookkeeping`

- Status: completed
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-048`, by testing one bounded solver-core candidate that only touches exact `sub10 step-3` learnt-large non-last deep-overwrite removals when the source watcher index is `8+`
- Assumptions:
  - `perf-046` already showed that the broader exact `index 7+` pop-first rewrite loses the dense anchors, focused seven-case slice, and supplemental `satlib_more` slice, so this run should keep exact `index 7`, exact `index 6`, exact `index 5`, exact `index 4`, exact `index 3`, exact `index 2`, shallow, and last-slot behavior on the retained baseline path.
  - `perf-047` narrowed the remaining measured deep-overwrite tail to exact source index `8+`, so this is the narrowest plausible solver-core follow-up that still covers the dominant remaining tail on both the dense anchors and the real supplemental target trio.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-048` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one exact `sub10 step-3` deep-overwrite `index 8+` bookkeeping candidate against the dense anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate while `perf-048` was active (`83/83` tests green plus compile/checker/wrapper smoke checks)
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf048_index8plus_baseline.rAhRxq/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- candidate rejected on the dense anchor pair two-order average (`20.9634s -> 21.1585s`), with the giveback split mostly between `large/test_6.cnf` and the reverse-order `special/hard.cnf`
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf048_index8plus_baseline.rAhRxq/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- candidate rejected on the focused seven-case gate (`25.7930s -> 26.0138s`), with dense-anchor losses outweighing smaller gains on `special/hard.cnf` reverse order and `large/test_10.cnf`
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf048_index8plus_baseline.rAhRxq/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- positive on the supplemental slice (`0.3433s -> 0.3305s`), led by `jnh10.cnf` and `uf125-01.cnf`, but not strong enough to overcome the primary-gate losses
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-049'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync (`83/83` tests green plus compile/checker/wrapper smoke checks)
- `git diff --check`
- passed after the final control-plane sync

### Outcome

- Closed `perf-048` as a retained no-op with no solver change.
- Tested one bounded solver-core candidate in `satsolver_core.py`, mirrored in `tools/profile_solver.py`, that applied the pop-first source-pop rewrite only to exact `sub10 step-3` learnt-large non-last deep-overwrite removals at source index `8+`, then reverted it after the primary early gates rejected it.
- The candidate did improve the supplemental `satlib_more` slice, but it still lost the dense anchor pair and the focused seven-case hotspot slice, so there was no reason to spend a repeat-aware exact-CLI full-suite run on it.
- The queue therefore advances to `perf-049`, which should stay measurement-only and split the exact `index 8+` deep-overwrite tail into exact source index `8` versus `index 9+` before another solver-core edit.

### Remaining risks

- The exact `index 8+` aggregate is still the narrowest measured remaining lane, but `perf-048` shows that even this aggregate can hide primary-gate losses. The next follow-up should not widen back out before splitting that tail once more.
...
- Task family: native-only learnt-large profiling refresh
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-045`, by profiling how the surviving exact `sub10 step-3` deep-overwrite `index 6+` lane splits between exact source index `6` and source index `7+` across the dense anchors and the supplemental `satlib_more` guard slice
- Assumptions:
  - `perf-044` already rejected the pop-first rewrite across the whole exact `index 6+` deep-overwrite aggregate, so this run should stay measurement-only and explain whether the remaining tail is really concentrated at exact index `6` or in the deeper `index 7+` tail.
  - The current profiler already separates exact `step-3` last-slot, overwrite, shallow, deep, exact `index 2`, aggregate `index 3+`, exact `index 3`, aggregate `index 4+`, exact `index 4`, aggregate `index 5+`, exact `index 5`, and aggregate `index 6+` cases, so the missing information should be a profiler-only split inside the `index 6+` bucket plus regression coverage.
  - A completed measurement-only outcome is valid if it names the dominant exact tail sublane and leaves the queue with one narrower next solver-core experiment.
- Escalations: none

### Plan

- [x] Mark `perf-045` in progress in the control plane and record the active profiling task in `PLANS.md`.
- [x] Add profiler-only exact `index 6` versus `index 7+` deep-overwrite counters plus regression coverage, then profile the dense anchors and supplemental guard slice.
- [x] Close the measurement run, queue the next deterministic task, verify the final state, and commit.

### Verification

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q`
- passed (`22/22` green, including the new exact `index 6` versus `index 7+` deep-tail split test)
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- passed and reported dense-anchor exact `index 6+` deep overwrites `8,385` at exact index `6` versus `17,756` at `index 7+`, plus supplemental target-trio `51` versus `160`; `jnh10` and `jnh1` stayed at zero exact `index 6+` hits
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-046'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync (`82/82` tests green plus compile, checker, queue, and wrapper smoke checks)
- `git diff --check`
- passed after the final control-plane sync

### Outcome

- Closed `perf-045` as a retained measurement-only profiling run with no solver change.
- Added profiler-only exact `index 6` versus `index 7+` counters inside the exact `sub10 step-3` deep-overwrite `index 6+` lane in `tools/profile_solver.py`, with regression coverage in `tests/test_profile_solver.py`.
- The surviving tail is still dominated by exact source index `7+`, not exact index `6`, on both the dense anchors and the real supplemental target trio: dense anchors split `26,141` total exact `index 6+` hits into `8,385` at exact index `6` versus `17,756` at `index 7+`, while the supplemental target trio split `211` into `51` versus `160`.
- `jnh10` and `jnh1` remained pure guardrails with zero exact `index 6+` hits, so the next deterministic queue task is `perf-046`, a bounded solver-core candidate that should touch only the exact `index 7+` tail.

### Remaining risks

- The exact `index 7+` tail is now the narrowest surviving solver-core lane, but it is only measured so far. The next candidate still has to clear the dense anchors, focused seven-case slice, supplemental `satlib_more` guard slice, and the repeat-aware exact-CLI keep gate before it can be retained.

## 2026-03-23 `perf-044-index6plus-deep-overwrite-bookkeeping`

- Status: completed
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-044`, by testing one bounded dense-UNSAT solver-core candidate that only touches exact `sub10 step-3` learnt-large non-last deep-overwrite removals when the source watcher index is `6+`
- Assumptions:
  - `perf-034`, `perf-036`, `perf-038`, `perf-040`, and `perf-042` already rejected the same pop-first source-pop rewrite across the broader exact non-last overwrite, `index 2+`, `index 3+`, `index 4+`, and `index 5+` aggregates, so this run should leave exact index `5`, exact index `4`, exact index `3`, exact index `2`, shallow, and last-slot behavior on the retained baseline path.
  - `perf-043` showed that the surviving exact `index 5+` tail is dominated by exact source index `6+` on both the dense anchors and the real supplemental target trio, so this is the narrowest plausible solver-core follow-up that still covers the dominant remaining tail.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-044` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one exact `sub10 step-3` deep-overwrite `index 6+` bookkeeping candidate against the dense anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate while `perf-044` was active: the queue stayed consistent, all `81/81` tests stayed green, and both default wrapper smoke paths remained valid
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf044_index6plus_baseline.ARThzg/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- rejected on the dense anchor pair two-order average (`23.8961s -> 24.8445s`), with the forward pass clearly worse on both anchors even though the reverse pass recovered some of the loss
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf044_index6plus_baseline.ARThzg/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- mildly positive on the focused seven-case gate (`29.6107s -> 29.5913s`), led by forward gains on `large/test_6.cnf` and `special/hard.cnf` with giveback on `large/test_10.cnf` and `medium/test_4.cnf`
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf044_index6plus_baseline.ARThzg/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- positive on the supplemental slice (`0.3624s -> 0.3473s`), with the strongest forward gain on `uf125-010.cnf` and stable help on both `jnh` guardrails
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf`
- passed on the temporary candidate and kept the dense hard-case search counters unchanged at `72,886/59,201` on `large/test_6.cnf` and `54,245/44,619` on `special/hard.cnf`, so the branch still looked like same-search bookkeeping rather than heuristic drift
- `python tools/codex_verify.py --benchmark-mode cli --repeat 2`
- passed as a correctness run but rejected as the final keep gate: the candidate stayed `59/59` correct yet regressed the repeat-aware exact-CLI suite from the same-day baseline `32.3903s` representative / `64.7805s` measured to `33.2616s` representative / `66.5232s` measured
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-045'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync
- `git diff --check`
- passed after the final control-plane sync
...
- Tested one bounded solver-core candidate in `satsolver_core.py` that applied the earlier pop-first source-pop rewrite only to exact `sub10 step-3` learnt-large non-last deep-overwrite removals at source index `6+`, mirrored in `tools/profile_solver.py`, then reverted it after the broad repeat-aware exact-CLI keep gate failed.
- The candidate stayed same-search on the dense anchors and did win the focused seven-case slice plus the supplemental `satlib_more` guard slice, but it still lost the stronger full-suite gate by `+0.8713s` representative (`32.3903s -> 33.2616s`).
- The broad-suite giveback concentrated back on the dense hard families rather than a new lane: `special/hard.cnf` regressed by about `+0.4392s`, `large/test_6.cnf` by `+0.3169s`, and `large/test_10.cnf` by `+0.1181s`, which outweighed the smaller wins on `satlib_more/uuf150-01.cnf`, `medium/test_3.cnf`, and parts of the supplemental guard slice.
- The queue therefore advances to `perf-045`, which should stay measurement-only and split the exact `index 6+` deep-overwrite tail into exact source index `6` versus `index 7+` before another solver-core edit.

### Remaining risks

- The exact `index 6+` tail is still the narrowest solver-core lane that produced positive focused-slice evidence, but `perf-044` shows it can still hide broad-suite losses. The next follow-up should not widen back out before splitting that tail once more.
...
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-042`, by testing one bounded dense-UNSAT solver-core candidate that only touches exact `sub10 step-3` non-last deep-overwrite removals when the source watcher index is `5+`
- Assumptions:
  - `perf-034`, `perf-036`, `perf-038`, and `perf-040` already rejected the same pop-first source-pop rewrite across the broader exact non-last overwrite, `index 2+`, `index 3+`, and `index 4+` aggregates, so this run should leave exact index `4`, exact index `3`, exact index `2`, shallow, and last-slot behavior on the retained baseline path.
  - `perf-041` showed that the surviving exact `index 4+` tail is dominated by exact source index `5+` on both the dense anchors and the real supplemental target trio, so this is the narrowest plausible solver-core follow-up that still covers the dominant remaining tail.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-042` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one exact `sub10 step-3` deep-overwrite `index 5+` bookkeeping candidate against the dense anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate while `perf-042` was active: the queue stayed consistent, all `80/80` tests stayed green, and both default wrapper smoke paths remained valid
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf042_index5plus_baseline.1gYnb9/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- slightly negative on the dense anchor pair two-order average (`22.4346s -> 22.5260s`), with `large/test_6.cnf` positive only in forward order and `special/hard.cnf` positive only in reverse order
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf042_index5plus_baseline.1gYnb9/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- rejected on the focused seven-case gate (`26.5830s -> 27.5761s`), with the main recurring damage on `large/test_6.cnf`, `special/hard.cnf`, `medium/test_4.cnf`, and `satlib_more/uuf150-01.cnf`
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf042_index5plus_baseline.1gYnb9/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- rejected on the supplemental slice too (`0.3316s -> 0.3512s`), with the clearest reverse-order giveback on `uf125-010.cnf` and additional losses on `uuf125-010.cnf` and `jnh10.cnf`
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-043'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync
- `git diff --check`
- passed after the final control-plane sync
...
- Tested one bounded solver-core candidate in `satsolver_core.py` that applied the earlier pop-first source-pop rewrite only to exact `sub10 step-3` learnt-large non-last deep-overwrite removals at source index `5+`, mirrored in `tools/profile_solver.py`, then reverted it after all three early gates stayed negative.
- The result sharpens the lane selection again: even the exact `index 5+` deep-overwrite aggregate is still too broad for a retained keep.
- The queue therefore advances to `perf-043`, which should stay measurement-only and split the exact `index 5+` deep-overwrite lane into exact source index `5` versus `index 6+` before another solver-core edit.

### Remaining risks

- The rejected `index 5+` aggregate still contains the dominant deep-overwrite volume, but `perf-042` shows that even this narrower bucket can still mix incompatible behavior across the dense anchors and supplemental guard slice. The next solver-core follow-up should not widen back out before this tail is split once more.

## 2026-03-23 `perf-041-index4plus-source-index-profile`

- Status: completed
- Task family: native-only learnt-large profiling refresh
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-041`, by profiling how the surviving exact `sub10 step-3` deep-overwrite `index 4+` lane splits between exact source index `4` and source index `5+` across the dense anchors and the supplemental `satlib_more` guard slice
- Assumptions:
  - `perf-040` already rejected the pop-first rewrite across the whole exact `index 4+` deep-overwrite aggregate, so this run should stay measurement-only and explain whether the remaining tail is really concentrated at exact index `4` or in the deeper `index 5+` tail.
  - The current profiler already separates exact `step-3` last-slot, overwrite, shallow, deep, exact `index 2`, aggregate `index 3+`, exact `index 3`, and aggregate `index 4+` cases, so the missing information should be a profiler-only split inside the `index 4+` bucket plus regression coverage.
  - A completed measurement-only outcome is valid if it names the dominant exact tail sublane and leaves the queue with one narrower next solver-core experiment.
- Escalations: none

### Plan

- [x] Mark `perf-041` in progress in the control plane and record the active profiling task in `PLANS.md`.
- [x] Add profiler-only exact `index 4` versus `index 5+` deep-overwrite counters plus regression coverage, then profile the dense anchors and supplemental guard slice.
- [x] Close the measurement run, queue the next deterministic task, verify the final state, and commit.

### Verification

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q`
- passed: the profiler test file stayed green at `20/20`, including a new natural-propagation case that forces one exact `index 4` deep overwrite and one exact `index 5+` deep overwrite inside the existing exact `sub10 step-3` lane
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- passed: the exact `index 4+` deep-overwrite tail is dominated by `index 5+` rather than exact `index 4` on both dense anchors and the real supplemental target trio. Dense anchors combined `17,482` exact index `4` versus `38,346` index `5+` deep overwrites (`31.32%` / `68.68%`), while `uuf125-010`, `uf125-01`, and `uf125-010` combined `146` versus `303` (`32.52%` / `67.48%`). `jnh10` and `jnh1` stayed at zero in the exact `index 4+` lane.
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-042'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync
- `git diff --check`
- passed after the final control-plane sync
...
- Added profiler-only exact `index 4` versus `index 5+` counters inside the existing exact `sub10 step-3` deep-overwrite `index 4+` bucket in `tools/profile_solver.py`, plus regression coverage in `tests/test_profile_solver.py`, so the repo can now separate the front of that tail from the deeper `index 5+` remainder.
- The tail question is now answered clearly. Across `large/test_6.cnf` and `special/hard.cnf` together, exact `index 4+` deep overwrites split `17,482` at exact index `4` versus `38,346` at `index 5+`. Across the supplemental target trio `uuf125-010`, `uf125-01`, and `uf125-010`, the split was `146` versus `303`.
- The queue therefore advances to `perf-042`, which should test one bounded solver-core candidate only on the exact `sub10 step-3` deep-overwrite `index 5+` tail while keeping exact index `4`, exact index `3`, exact index `2`, shallow, and last-slot behavior on the retained baseline path.

### Remaining risks

- The `index 5+` tail is now clearly dominant, but `perf-041` is still measurement-only. The next solver-core candidate must still satisfy the dense anchors, the focused seven-case hotspot slice, the supplemental `satlib_more` guard slice, and then the repeat-aware exact-CLI keep gate before any retainable solver change lands.

## 2026-03-23 `perf-040-index4plus-deep-overwrite-bookkeeping`

- Status: completed
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-040`, by testing one bounded dense-UNSAT solver-core candidate that only touches exact `sub10 step-3` non-last deep-overwrite removals when the source watcher index is `4+`
- Assumptions:
  - `perf-034`, `perf-036`, and `perf-038` already rejected the same pop-first rewrite across the broader exact non-last overwrite, `index 2+`, and `index 3+` aggregates, so this run should leave exact index `3`, exact index `2`, shallow, and last-slot behavior on the retained baseline path.
  - `perf-039` showed that the surviving exact `index 3+` tail is dominated by exact source index `4+` on both the dense anchors and the real supplemental target trio, so this is the narrowest plausible solver-core follow-up that still covers the dominant remaining tail.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the broader repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-040` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one exact `sub10 step-3` deep-overwrite `index 4+` bookkeeping candidate against the dense anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate while `perf-040` was active: the queue stayed consistent, all `79/79` tests stayed green, and both default wrapper smoke paths remained valid
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf040_index4plus_baseline.f0x6rd/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- passed as an early signal on the dense anchor pair: the two-order average improved from `22.4207s` to `22.3468s`, with `large/test_6.cnf` positive in both orders and `special/hard.cnf` splitting direction
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf040_index4plus_baseline.f0x6rd/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- passed on the focused seven-case gate: the two-order average improved from `27.4240s` to `27.0142s`, led by a strong reverse-order `large/test_6.cnf` win and small gains on `special/hard.cnf` forward plus `large/test_8.cnf`
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf040_index4plus_baseline.f0x6rd/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- rejected on the supplemental slice: the two-order average regressed from `0.3462s` to `0.3530s`, with the main giveback on `jnh1.cnf`, `jnh10.cnf`, and reverse `uuf125-010.cnf`
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf`
- passed on the temporary candidate and kept the dense hard-case search counters unchanged at `72,886/59,201` on `large/test_6.cnf` and `54,245/44,619` on `special/hard.cnf`, so this branch still looked like same-search bookkeeping rather than heuristic drift
- `python tools/codex_verify.py --benchmark-mode cli --repeat 2`
- passed as a correctness run but rejected as the final keep gate: the candidate stayed `59/59` correct yet regressed the repeat-aware exact-CLI suite from the same-day baseline `30.4636s` representative / `60.9271s` measured to `31.6535s` representative / `63.3070s` measured
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-041'`
- `python tools/codex_verify.py`
- passed after the final control-plane sync (`79/79` tests green plus compile/checker/wrapper smoke checks)
- `git diff --check`
- passed after the final control-plane sync
...
- Tested one bounded solver-core candidate in `satsolver_core.py` that applied the earlier pop-first overwrite rewrite only to exact `sub10 step-3` learnt-large deep-overwrite removals at source index `4+`, mirrored in `tools/profile_solver.py`, then reverted it after the broad repeat-aware exact-CLI keep gate failed.
- The candidate stayed same-search on the dense anchors and did win both the dense anchor pair and the focused seven-case gate, but it still lost the supplemental `satlib_more` slice and then lost the stronger repeat-aware exact-CLI suite by `+1.1899s` representative (`30.4636s -> 31.6535s`).
- The broad-suite damage was concentrated back on the dense mixed guardrails rather than on a brand-new family: `special/hard.cnf` regressed by about `+0.5900s`, `large/test_6.cnf` by `+0.2871s`, `medium/test_3.cnf` by `+0.0788s`, `large/test_10.cnf` by `+0.0619s`, and `large/test_8.cnf` by `+0.0274s`.
- The queue therefore advances to `perf-041`, which should stay measurement-only and split the exact `index 4+` deep-overwrite tail into exact source index `4` versus `index 5+` before another solver-core edit.

### Remaining risks

- The exact `index 4+` tail is still a live lane because `perf-040` improved the primary hotspot gates while staying same-search, but this run proves the aggregate is still too broad for a retained keep. The next candidate needs a narrower selector than “all exact `index 4+` deep overwrites.”

## 2026-03-23 `perf-039-index3plus-source-index-profile`

- Status: completed
- Task family: native-only learnt-large profiling refresh
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-039`, by profiling how the surviving exact `sub10 step-3` deep-overwrite `index 3+` lane splits between exact source index `3` and source index `4+` across the dense anchors and the supplemental `satlib_more` guard slice
- Assumptions:
  - `perf-038` already rejected the pop-first rewrite across the whole exact `index 3+` deep-overwrite aggregate, so this run should stay measurement-only and explain whether the remaining tail is really concentrated at exact index `3` or in the deeper `index 4+` tail.
  - The current profiler already separates exact `step-3` last-slot, overwrite, shallow, deep, exact `index 2`, and aggregate `index 3+` cases, so the missing information should be a profiler-only split inside the `index 3+` bucket plus regression coverage.
  - A completed measurement-only outcome is valid if it names the dominant exact tail sublane and leaves the queue with one narrower next solver-core experiment.
- Escalations: none

### Plan

- [x] Mark `perf-039` in progress in the control plane and record the active profiling task in `PLANS.md`.
- [x] Add profiler-only exact `index 3` versus `index 4+` deep-overwrite counters plus regression coverage, then profile the dense anchors and supplemental guard slice.
- [x] Close the measurement run, queue the next deterministic task, verify the final state, and commit.

### Verification

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q`
- passed: the profiler test file stayed green at `19/19`, including a new natural-propagation case that forces one exact deep overwrite at source index `3` and one at source index `4+`
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- passed: the exact `index 3+` deep-overwrite lane is dominated by the deeper `index 4+` tail rather than exact index `3` on both dense anchors and the real supplemental target trio. Dense anchors combined `23,468` exact index `3` versus `55,828` index `4+` deep overwrites (`29.60%` / `70.40%`), while `uuf125-010`, `uf125-01`, and `uf125-010` combined `200` versus `449` (`30.82%` / `69.18%`)
- `python tools/codex_verify.py`
- passed after the final control-plane sync (`79/79` tests green plus compile/checker/wrapper smoke checks)
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-040'`
- `git diff --check`
- passed after the final control-plane sync

### Outcome

- Closed `perf-039` as a measurement-only profiling run with no solver change.
- Added profiler-only exact `index 3` versus `index 4+` counters inside the existing exact `sub10 step-3` deep-overwrite `index 3+` bucket in `tools/profile_solver.py`, plus regression coverage in `tests/test_profile_solver.py`, so the repo can now separate the front of the deep tail from the deeper `index 4+` remainder.
- The tail question is now answered clearly. Across `large/test_6.cnf` and `special/hard.cnf` together, exact `index 3+` deep overwrites split `23,468` at exact index `3` versus `55,828` at `index 4+`. Across the supplemental target trio `uuf125-010`, `uf125-01`, and `uf125-010`, the split was `200` versus `449`.
- The queue therefore advances to `perf-040`, which should test one bounded solver-core candidate only on the exact `sub10 step-3` deep-overwrite `index 4+` tail while keeping exact index `3`, exact index `2`, shallow, and last-slot behavior on the retained baseline path.

### Remaining risks

- The `index 4+` tail is now clearly dominant, but `perf-039` is still measurement-only. The next solver-core candidate must still satisfy the dense anchors, the focused seven-case hotspot slice, and the supplemental `satlib_more` guard slice before any keep.

## 2026-03-23 `perf-038-index3plus-deep-overwrite-bookkeeping`

- Status: completed
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-038`, by testing one bounded dense-UNSAT solver-core candidate that only touches exact `sub10 step-3` non-last overwrite removals when the source watcher index is `3+`
- Assumptions:
  - `perf-034` rejected the pop-first rewrite across the whole exact `step-3` non-last overwrite lane, and `perf-036` rejected it again across the broader `index 2+` deep aggregate, so this run should leave exact index `2`, shallow, and last-slot behavior on the retained baseline path.
  - `perf-037` showed that the surviving exact deep-overwrite lane is dominated by source index `3+` on both the dense anchors and the real supplemental target trio, so this is the narrowest plausible solver-core follow-up that still covers the dominant deep tail.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the broader repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-038` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one exact `index 3+` deep-overwrite bookkeeping candidate against the dense anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate while `perf-038` was active: the queue stayed consistent, all `78/78` tests stayed green, and both default wrapper smoke paths remained valid
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf038_index3plus_baseline.MONJWd/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- rejected on the dense anchor pair two-order average (`22.0216s -> 22.5323s`), with `special/hard.cnf` clearly worse in both orders and `large/test_6.cnf` splitting direction
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf038_index3plus_baseline.MONJWd/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- rejected on the focused seven-case gate (`27.4716s -> 27.6531s`), with the main recurring damage on `special/hard.cnf`, `large/test_8.cnf`, and `satlib_more/uuf150-01.cnf`
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf038_index3plus_baseline.MONJWd/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- rejected on the supplemental slice too (`0.3402s -> 0.3510s`), with the clearest losses on `jnh10.cnf` and `uf125-01.cnf`
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-039'`
- `python tools/codex_verify.py`
- passed after reverting the candidate and syncing the control plane (`78/78` tests green plus compile/checker/wrapper smoke checks)
- `git diff --check`
- passed after the final control-plane sync
...
- Tested one bounded dense-anchor candidate in `satsolver_core.py` that applied the earlier pop-first exact `step-3` rewrite only to exact deep-overwrite removals at source index `3+`, while leaving exact index `2`, shallow, and last-slot behavior on the retained baseline path, then reverted it after all early gates stayed negative.
- The result sharpens the lane selection again: even the exact `index 3+` deep-overwrite aggregate is still too broad for a retained keep.
- The queue therefore advances to `perf-039`, which should stay measurement-only and split the exact `index 3+` deep-overwrite lane into exact index `3` versus index `4+` before another solver-core edit.

### Remaining risks

- The rejected `index 3+` aggregate still contains the dominant deep-overwrite volume, but `perf-038` shows that even this narrower bucket can still mix incompatible behaviors across the dense anchors and supplemental guard slice.

## 2026-03-23 `perf-037-deep-overwrite-source-index-profile`

- Status: completed
- Task family: native-only learnt-large profiling refresh
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-037`, by profiling how the surviving exact `sub10 step-3` deeper overwrite lane splits between source index `2` and source index `3+` across the dense anchors and the supplemental `satlib_more` guard slice
- Assumptions:
  - `perf-036` already rejected the pop-first rewrite across the whole deeper `index 2+` exact `step-3` overwrite aggregate, so this run should stay measurement-only and explain whether the remaining lane is really concentrated at exact index `2` or in the deeper `index 3+` tail.
  - The current profiler already separates exact `step-3` last-slot, overwrite, shallow, and deep cases, so the missing information should be a profiler-only split inside the deep overwrite bucket plus regression coverage.
  - A completed measurement-only outcome is valid if it names the dominant exact deep-overwrite sublane and leaves the queue with one narrower next solver-core experiment.
- Escalations: none

### Plan

- [x] Mark `perf-037` in progress in the control plane and record the active profiling task in `PLANS.md`.
- [x] Add profiler-only exact deep-overwrite source-index counters plus regression coverage, then profile the dense anchors and supplemental guard slice.
- [x] Close the measurement run, queue the next deterministic task, verify the final state, and commit.

### Verification

- `python -m unittest discover -s tests -p 'test_profile_solver.py' -q`
- passed: the profiler test file stayed green at `18/18`, including a new natural-propagation case that forces one exact deep overwrite at source index `2` and one at source index `3+`
- `python tools/profile_solver.py large/test_6.cnf special/hard.cnf satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- passed: the exact deep overwrite lane is dominated by source index `3+` rather than exact index `2` on both dense anchors and the real supplemental target trio. Dense anchors combined `29,299` exact index `2` versus `79,296` exact index `3+` deep overwrites (`26.98%` / `73.02%`), while `uuf125-010`, `uf125-01`, and `uf125-010` combined `250` versus `649` (`27.81%` / `72.19%`)
- `python tools/codex_verify.py`
- passed after the final control-plane sync (`78/78` tests green plus compile/checker/wrapper smoke checks)
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-038'`
- `git diff --check`
- passed after the final control-plane sync

### Outcome

- Closed `perf-037` as a measurement-only profiling run with no solver change.
- Added profiler-only exact deep-overwrite source-index counters inside the existing exact `sub10 step-3` deep overwrite bucket in `tools/profile_solver.py`, plus regression coverage in `tests/test_profile_solver.py`, so the repo can now separate exact source index `2` from the deeper `index 3+` tail.
- The source-index question is now answered clearly. Across `large/test_6.cnf` and `special/hard.cnf` together, exact deep overwrites split `29,299` at exact index `2` versus `79,296` at `index 3+`. Across the supplemental target trio `uuf125-010`, `uf125-01`, and `uf125-010`, the split was `250` versus `649`.
- The queue therefore advances to `perf-038`, which should test one bounded solver-core candidate only on the exact `sub10 step-3` deep overwrite `index 3+` sublane while keeping exact index `2`, shallow, and last-slot behavior on the retained baseline path.

### Remaining risks

- The `index 3+` deep lane is now clearly dominant, but `perf-037` is still measurement-only. The next solver-core candidate must satisfy the dense anchors, the focused seven-case hotspot slice, and the supplemental `satlib_more` guard slice before any keep.

## 2026-03-23 `perf-036-deeper-step3-overwrite-bookkeeping`

- Status: completed
- Task family: native-only learnt-large propagation bookkeeping experiment
- Branch/worktree: current checkout
- Prompt summary: continue the next deterministic queue task, `perf-036`, by testing one bounded dense-UNSAT solver-core candidate that only touches exact `sub10 step-3` non-last overwrites at watcher-list index `2+` on `special/hard.cnf` and `large/test_6.cnf`
- Assumptions:
  - `perf-034` already rejected the pop-first rewrite across the whole exact `step-3` non-last overwrite lane, so this run should keep shallow head-of-list overwrites on the retained path and only change deeper-slot removals.
  - `perf-035` showed that about two thirds of the dense-anchor exact `step-3` overwrite lane sits at watcher-list index `2+`, so this is the narrowest plausible follow-up that still covers the dominant overwrite tail.
  - A retained-noop outcome is valid if any early gate rejects the candidate before the broader repeat-aware exact-CLI suite.
- Escalations: none

### Plan

- [x] Mark `perf-036` in progress in the control plane and record the active bounded experiment in `PLANS.md`.
- [x] Implement and benchmark one deeper-slot exact `sub10 step-3` dense-anchor bookkeeping candidate against the anchor pair, focused seven-case slice, and supplemental `satlib_more` guard slice.
- [x] Keep or revert the candidate based on same-day evidence, then sync the control plane, verify, and commit.

### Verification

- `python tools/codex_verify.py`
- passed on the temporary candidate while `perf-036` was active: the queue stayed consistent, all `77/77` tests stayed green, and both default wrapper smoke paths remained valid
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf036_deepoverwrite_baseline.H28kf0/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf`
- rejected on the dense anchor pair two-order average (`22.0125s -> 22.0433s`), even though the forward order alone was slightly positive
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf036_deepoverwrite_baseline.H28kf0/satsolver.py --candidate-cli-script satsolver.py large/test_6.cnf special/hard.cnf large/test_10.cnf medium/test_4.cnf medium/test_3.cnf satlib_more/uuf150-01.cnf large/test_8.cnf`
- rejected on the focused seven-case gate (`26.7868s -> 27.2767s`), with the main damage from `large/test_6.cnf`, `large/test_10.cnf`, and `large/test_8.cnf`
- `python tools/hotspot_compare.py --baseline-cli-script /tmp/perf036_deepoverwrite_baseline.H28kf0/satsolver.py --candidate-cli-script satsolver.py satlib_more/uuf125-010.cnf satlib_more/uf125-01.cnf satlib_more/uf125-010.cnf satlib_more/jnh10.cnf satlib_more/jnh1.cnf`
- passed as a secondary signal only (`0.3581s -> 0.3406s`), which was not enough to override the negative primary gate
- `python tools/agent_queue_check.py`
- passed after the final control-plane sync; the queue now resolves to `current_or_next_task='perf-037'`
- `python tools/codex_verify.py`
- passed after reverting the candidate and syncing the control plane (`77/77` tests green plus compile/checker/wrapper smoke checks)
- `git diff --check`
- passed after the final control-plane sync
...
- Tested one bounded dense-anchor candidate in `satsolver_core.py` that applied the earlier pop-first exact `step-3` rewrite only to deeper watcher-list positions (`index 2+`) while leaving shallow head-of-list overwrites on the retained path, then reverted it after the primary early gates stayed negative.
- The result sharpens the lane selection: the deeper `index 2+` overwrite aggregate is still too broad. It can help the supplemental `satlib_more` slice while still losing on the dense anchors and the focused seven-case hotspot slice.
- The queue therefore advances to `perf-037`, which should stay measurement-only and split the deeper exact `step-3` overwrite lane by exact source index (`index 2` versus `index 3+`) before another solver-core edit.

### Remaining risks

- The positive supplemental slice means there may still be a surviving deep-overwrite sublane, but `perf-036` shows that treating all `index 2+` exact `step-3` overwrites as one bucket is too coarse for a retained keep.
